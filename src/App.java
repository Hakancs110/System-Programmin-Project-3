...
class Cache {
	...
	public int writeCacheContents(String address, String data) {
    	...
    	i = (sets_bits == 0 ? 0: 
    		Integer.parseInt(addressBinary.substring(Integer.parseInt(tagTemp), Integer.parseInt(tagTemp)+sets_bits),2));
        ...
        for(int i = 0; i < sets; i++) {
            for(int j = 0; j < lines; j++) {
                if(cacheObject[i][j] != null) {
                    System.out.println(cacheObject[i][j].getTag() + " " +cacheObject[i][j].getTime() + " " + cacheObject[i][j].getValid() + " " +  " " + cacheObject[i][j].getData());
                }
            }
        }
    }
...
public class App {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.printf("L1I-hits:%d L1I-misses:%d L1I-evicitons:%d\n", L1I.hits_counter, L1I.misses_counter, L1I.evictions_counter);
        System.out.printf("L1D-hits:%d L1D-misses:%d L1D-evicitons:%d\n", L1D.hits_counter, L1D.misses_counter, L1D.evictions_counter);
        System.out.printf("L2-hits:%d L2-misses:%d L2-evicitons:%d\n", L2.hits_counter, L2.misses_counter, L2.evictions_counter);
